...
- Chapter: `Rectification Improves Sparse Longitudinal Selection (RQ1)`; Section: `Approach`; Line: `Chapters/05_rq1.tex:40`; Relevance: Cited to support the statement that The sparse learner is intentionally ordinary: an L1-regularized logistic model is fit on the rectified design and compared against a matched raw lag-expanded sparse baseline.
- Chapter: `Anytime Rule Compression (RQ3)`; Section: `Approach`; Line: `Chapters/07_rq3.tex:35`; Relevance: Cited to support the statement that where $tilde x_{ij}$ is the binarized/rectified feature and $beta_j$ is sparse due to the L1 penalty.
