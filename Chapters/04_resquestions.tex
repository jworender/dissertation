...
\section{RQ1: Does Rectification Improve Sparse Longitudinal Selection?}
...
\section{RQ2: What Is the Reason for the Improvements Rectification Brings?}
...
At a high level, the motivating hypothesis is that rectification can contract harmful dependence structure in ways that improve sparse-support conditions related to IC-style reasoning \autocites{PZhao2006model,RAHorn2012Matrix}. Prior tractable analysis in this line supports this direction under explicit assumptions, but does not establish universal guarantees across all threshold settings and dependence regimes \autocite{JOrender2025Efficient}. RQ2 is therefore critical: it separates durable methodological insight from dataset-specific heuristic success.

RQ2 also matters for how results are interpreted across the dissertation. If gains appear only in certain dependence geometries or event-logic structures, that boundary must be made explicit. A clear mechanism and boundary analysis prevents overclaiming and supports reproducible, domain-appropriate deployment.

\section{RQ3: Can Sparse Rectified Models Become Logical Rule Sets?}
...
Detailed experimental designs are discussed elsewhere in the dissertation, but the work applies a common set of evaluation priorities across all three research questions:
\begin{itemize}
    ...
    \item \textbf{Practical non-inferiority framing where appropriate:} when simplified rule models are compared to upstream sparse baselines, prespecified non-inferiority margins can be used to assess whether performance loss is practically small \autocite{DJSchuirmann1987comparison}.
\end{itemize}
