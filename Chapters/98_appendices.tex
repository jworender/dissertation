...
\begin{figure}[htbp!]
	...
	\caption[Toy Example Coefficient Vectors.]{Coefficient vectors for LASSO on the same data before (top) and after (bottom) binarization. The non-zero feature coefficients indicate the selected features.}
...
After transformation, the matrix is encoded with ones and negative ones. The encoding rule in this case was simple: values between 0.4 and 0.6 (inclusive) became ones; others became negative ones. Details on defining this \textit{critical range} are in the other sections. A summarized version of the result is that if both features A and D were between 0.4 and 0.6 simultaneously, an event occurred.
...
\subsection{Intuition Development}

In the following examples, I show an easy way to visualize the logical process that helps develop an intuition for what types of problems this solution method works well for.  A binary outcome is derived from a continuous feature measured over time. Events occur when the feature enters a specific range, shown as the shaded area in Figure \ref{fig:first_irrelevant}. This behavior is common in physical phenomena that depend on temperature or pressure,  with phase changes being an obvious example. Collecting more data refines the event range. Even if only one relevant feature exists among many, defining binary values for all features highlights the causative feature. By replacing continuous measurements with binary values, the relevant feature becomes evident. If only one feature is relevant, its binary vector matches the result vector.
...
Generalizing this procedure to include situations in which an unknown number of features are required to simultaneously be within their own critical ranges for an event to occur is where the LASSO algorithm will be of use.
