...
\begin{equation}\label{eq:rq2-irrepresentable-condition}
...
\begin{equation}\label{eq:rq2-sign-binarization}
...
\paragraph{Proof of Concept (PoC) Assumptions.}
\begin{itemize}
...
Under bivariate normality, sign-binarized correlation is a deterministic function of raw Pearson correlation $(\rho)$, as shown in lemma 1:
\begin{equation}\label{eq:rq2-arcsin-correlation-map}
...
\begin{lemma}[Arcsin correlation map]\label{lem:rq2-arcsin-map}
Let $(X,Y)$ be a centered, variance-one bivariate normal pair with correlation $\rho$, and define $\tilde{X}=\operatorname{sign}(X)$ and $\tilde{Y}=\operatorname{sign}(Y)$. Then the transformed correlation follows the classical arcsine relation \autocite{WFSheppard1899Application}:
\begin{equation}\label{eq:rq2-lemma1-arcsin-map}
\tilde{\rho} = \frac{2}{\pi}\arcsin(\rho).
\end{equation}
\end{lemma}

\begin{proof}
Because $(X,Y)$ is bivariate normal and centered, the classical quadrant formula gives \autocite{WFSheppard1899Application}
\begin{equation}\label{eq:rq2-proof-positive-quadrant-probability}
P(X>0,Y>0)=\frac{1}{4}+\frac{\arcsin(\rho)}{2\pi}.
\end{equation}
For sign variables, $E[\tilde{X}]=E[\tilde{Y}]=0$ and
$E[\tilde{X}\tilde{Y}]=P(\tilde{X}=\tilde{Y})-P(\tilde{X}\neq\tilde{Y})$.
By symmetry, substituting the quadrant probability gives
$E[\tilde{X}\tilde{Y}]=4P(X>0,Y>0)-1=(2/\pi)\arcsin(\rho)$.
Since the sign variables have unit variance, this expectation is their correlation. Appendix B.1 gives the step-by-step derivation, including the equivalent $0/1$ encoding.
\end{proof}
...
\begin{lemma}[Pairwise contraction]\label{lem:rq2-correlation-contraction}
Under the assumptions of Lemma~\ref{lem:rq2-arcsin-map},
\begin{equation}\label{eq:rq2-lemma2-correlation-contraction}
|\tilde{\rho}| \leq |\rho|,
\end{equation}
...
\begin{proof}
The map $\arcsin(\rho)$ is odd, so it is enough to consider $0\leq\rho\leq1$. Let $\alpha=\arcsin(\rho)$, so $\rho=\sin(\alpha)$ with $\alpha\in[0,\pi/2]$. Concavity of sine on this interval implies $\sin(\alpha)\geq 2\alpha/\pi$, the chord from $(0,0)$ to $(\pi/2,1)$. Therefore $(2/\pi)\arcsin(\rho)\leq\rho$. Odd symmetry gives the same magnitude bound for negative $\rho$. Appendix B.2 gives the expanded case analysis.
\end{proof}

\textbf{Interpretation:} The transformed correlation magnitude is bounded by the raw magnitude. This bound is the key contraction property used downstream to control leakage and reduce amplification risk in de-mixing.

\begin{lemma}[Inactive-active leakage contraction]\label{lem:rq2-leakage-contraction}
Let $x_{{S^c}i}$ be an inactive standardized Gaussian feature and $x_{Sj}$ an active standardized Gaussian feature satisfying the pairwise assumptions above. For their sign-binarized counterparts,
\begin{equation}\label{eq:rq2-lemma3-leakage-contraction}
...
\begin{proof}
The raw variables are standardized, so their covariance equals their correlation. The sign-binarized variables are centered and have unit variance under the symmetric Gaussian assumption, so their covariance also equals their correlation. Applying Lemma~\ref{lem:rq2-correlation-contraction} to each inactive-active pair gives the stated covariance bound. Appendix B.3 expands this pairwise argument for the inactive-active covariance block.
\end{proof}
...
\begin{lemma}[Active-block inverse control]\label{lem:rq2-active-inverse-control}
In the positive-correlation analyzed regime, assume the active block is equicorrelated with support size $s$ and correlation $0\leq\rho<1$:

\begin{equation}\label{eq:rq2-active-equicorrelation-gram}
G=(1-\rho)I_s+\rho J_s.
\end{equation}
...
\begin{equation}\label{eq:rq2-lemma4-active-inverse-bound}
\|\tilde{G}^{-1}\|_\infty \leq \|G^{-1}\|_\infty.
\end{equation}
\end{lemma}

\begin{proof}
For an equicorrelation matrix, Sherman-Morrison gives
\begin{equation}\label{eq:rq2-equicorrelation-inverse}
G^{-1}=\frac{1}{1-\rho}I_s-\frac{\rho}{(1-\rho)(1+(s-1)\rho)}J_s,
\end{equation}
and the same expression holds for $\tilde{G}^{-1}$ after replacing $\rho$ with $\tilde{\rho}$. In the positive-correlation regime, the resulting row-sum norm is monotone increasing in $\rho$ on $[0,1)$. Lemma~\ref{lem:rq2-correlation-contraction} gives $0\leq\tilde{\rho}\leq\rho$, so the transformed inverse row-sum norm is no larger than the raw one. Appendix B.4 contains the expanded matrix calculation.
\end{proof}

\textbf{Interpretation:} The transformed active block requires no greater inverse amplification than the raw block in the target regime. Combined with Lemma~\ref{lem:rq2-leakage-contraction}, this yields a smaller IC interference term and therefore a higher chance of satisfying the IC margin.

\section{Theorem and Proof}

\begin{theorem}[Sufficient IC-bound probability under sign binarization]\label{thm:rq2-ic-probability}
Let
\begin{equation}\label{eq:rq2-theta-definition}
...
\begin{align}
B_i
&=\|\mathbf{c}_i\|\,\|\mathbf{G}^{-1}\|\,\|\operatorname{sign}(\boldsymbol{\beta}_S)\|,
\label{eq:rq2-raw-ic-bound}\\
\tilde{B}_i
&=\|\tilde{\mathbf{c}}_i\|\,\|\tilde{\mathbf{G}}^{-1}\|\,\|\operatorname{sign}(\boldsymbol{\beta}_S)\|.
\label{eq:rq2-binarized-ic-bound}
\end{align}
and set $B_\star=\max_{i\in S^c}B_i$ and $\tilde{B}_\star=\max_{i\in S^c}\tilde{B}_i$.
Assume the PoC conditions, the positive equicorrelation regime of Lemma~\ref{lem:rq2-active-inverse-control}, and the large-sample covariance approximation used in this chapter. Then sign binarization weakly improves the probability that this sufficient IC bound certifies the margin:
\begin{equation}\label{eq:rq2-theta-probability-improvement}
P\big(B_\star<1\big) \leq P\big(\tilde{B}_\star<1\big).
\end{equation}
\end{theorem}

\begin{proof}
For each inactive feature index $i\in S^c$, define its IC interference score
\begin{equation}\label{eq:rq2-proof-inactive-interference-score}
\theta_i = \left|\mathbf{c}_i\mathbf{G}^{-1}\operatorname{sign}(\boldsymbol{\beta}_S)\right|,
\end{equation}
where $\mathbf{c}_i$ is its covariance vector with the active features and $\mathbf{G}$ is the active Gram matrix. In the large-sample regime, sample Gram and covariance blocks concentrate around their population counterparts, so raw and binarized designs can be compared through $(\boldsymbol{\rho}_{iS},\boldsymbol{\rho}_{SS})$ and $(\tilde{\boldsymbol{\rho}}_{iS},\tilde{\boldsymbol{\rho}}_{SS})$.

Lemma~\ref{lem:rq2-leakage-contraction} contracts each inactive-active covariance term, giving a no-larger transformed leakage vector in the analyzed norm. Lemma~\ref{lem:rq2-active-inverse-control} gives no greater active-block inverse amplification in the positive/equicorrelated regime. Therefore, by submultiplicativity,
\begin{equation}\label{eq:rq2-proof-binarized-interference-bound}
\theta_i^{\text{bin}}
\leq
\|\tilde{\mathbf{c}}_i\|\,\|\tilde{\mathbf{G}}^{-1}\|\,\|\operatorname{sign}(\boldsymbol{\beta}_S)\|
\leq
...
All studies presented use controlled synthetic covariance families rather than one fixed observed dataset. That choice is deliberate. RQ2 is about mechanism, so direct control is needed over the dependence geometry that drives IC behavior. The baseline construction fixes an active support of size $s=4$ (except in the case of the heat map in figure \ref{fig:rq2_ic_heatmaps}, in which case $s$ is expanded to 25 in order to remain consistent with the HAI dataset that is plotted over it) and begins with one inactive feature. The active block is equicorrelated with parameter $\rho_{\text{active}}$, and the inactive-active coupling is controlled by a multiplier $c$, which induces
\begin{equation}\label{eq:rq2-boundary-cross-correlation-scaling}
...
\begin{equation}\label{eq:rq2-boundary-ic-proxy}
...
\begin{figure}[tbh]
    ...
    \caption[RQ2 harder IC boundary heatmaps.]{Harder IC boundary maps over active-block correlation and inactive-active coupling. The top row shows mean $\Theta$ and the white contour marks the operative failure boundary $\Theta=1$. The bottom row reports the empirical safe frequency $P(\Theta<1)$. Rectification pushes the failure boundary outward but does not eliminate failure. }
    \label{fig:rq2_ic_heatmaps}
\end{figure}

\begin{figure}[tbh]
	\centering
	\includegraphics[width=\textwidth]{Figures/boundary_ic_hai_a2_rectified_calibrated_heatmaps.png}
	\caption[RQ2 harder IC boundary heatmaps with HAI overlay.]{Harder IC boundary maps over active-block correlation and inactive-active coupling expanded to s=25 and with the HAI data included. As with figure \ref{fig:rq2_ic_heatmaps}, the top row shows mean $\Theta$ and the white contour marks the operative failure boundary $\Theta=1$. The bottom row reports the empirical safe frequency $P(\Theta<1)$. The box-plot overlay shows the real-world HAI dataset, and illustrates the effect of the process of binarization. }
	\label{fig:rq2_ic_heatmaps_w_hai}
\end{figure}

Figure \ref{fig:rq2_ic_heatmaps} addresses the most important objection to the baseline sweep: perhaps the original coupling level was simply too mild. The heatmaps vary both $\rho_{\text{active}}$ and $c$ (the cross-scale factor) and therefore expose the actual rectified failure boundary. The top row shows mean $\Theta$ itself. In these panels, the white contour is the operational threshold $\Theta=1$. Everything below that contour is IC-favorable in the proxy sense, and everything above it is not. The visual comparison is therefore direct: the method with the larger dark region below the white contour retains IC favorability over more of the parameter space.
...
The answer to RQ2 is \textbf{yes, in a scoped form}. Rectification can improve sparse support recovery because, in the analyzed regime, binarization changes the correlation geometry that drives inactive-active interference. In the zero-threshold, jointly normal, positive-correlation setting, sign binarization yields the arcsine correlation map, contracts pairwise dependence, reduces the inactive-active leakage term, and weakly improves the probability that a sufficient IC bound certifies the support-recovery margin.
