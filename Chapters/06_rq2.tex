...
The closed-form theorem is intentionally conservative. The dissertation therefore supplements it with empirical boundary checks that test how the mechanism behaves when its simplifying assumptions are relaxed. The corresponding threshold-perturbation studies, correlation-pattern sweeps, and negative-correlation summaries are reported in \texttt{<<< deficiency D003 >>>}.
\begin{enumerate}
    \item Correlation-pattern sweeps track how support stability changes as the design approaches known IC failure regimes.
    \item Threshold-placement perturbation studies measure how robust rectification remains when practical critical ranges depart from the idealized zero-threshold setting.
    \item Negative-correlation and complement-feature cases identify where correlation contraction weakens or becomes asymmetric.
    \item Cross-dataset comparisons separate settings where the theorem's mechanism predicts observed gains from those where the benefits are primarily interpretability-driven.
\end{enumerate}

\section{Empirical Implications of RQ2}

The theorem's implications appear in the dissertation results in three recurring patterns:
\begin{enumerate}
    \item Under controlled dependence stress, transformed models show stronger support stability than their raw counterparts \texttt{<<< deficiency D003 >>>}.
    \item In threshold-aligned regimes, false-positive lag selection drops as inactive-feature leakage is reduced.
    \item Failure cases cluster where threshold mismatch or adverse covariance geometry breaks the conditions that make correlation contraction most helpful.
\end{enumerate}

These empirical patterns convert RQ2 from an abstract theorem into a concrete explanation for the behavior observed in RQ1 and RQ3.
...
In summary, the theorem does not end the theoretical story, but it establishes a coherent bridge from binarization to IC favorability. That bridge is sufficient to support the dissertation's mechanistic claim while also identifying the boundary conditions that matter for later extensions.
